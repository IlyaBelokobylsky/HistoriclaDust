...
(function() {
    ...
    function takeTest() {
        let difficult,
            ...
            [
                ...
                new CenturyQuestion('Назовите дату начала войны <br> за независимость США.', 18, 'век'),
            ...
            [
                ...
                new StringQuestion('Где возник первый очаг новой мировой <br> войны в 1931 году?', 'Маньчжурия', ['Польша', 'Чехословакия', 'Австрия']),
        ...
        const continueDisplay = document.querySelector('.test_show-answer'),
            endDisplay = document.querySelector('.test__show-score');
        ...
        function _endTest() {
            const scoreElem = document.querySelector('.test__end-score'),
                lvlElem = document.querySelector('.test__show-level'),
                lvls = [
                    // easy
                    {
                        bad: 'Судя по всему, вы только начинаете изучать историю. ' +
                            'Вам следует немного покопаться по данному сайту.',
                        medium: 'Вы уже чуть-чуть разбираетесь в азах истории. ' +
                            'Этот сайт поможет Вам разобраться глубже!',
                        good: 'Вы уже практически изучили азы истории. ' +
                            'Этот сайт поможет вам закрыть дыры в материале!',
                        awesome: 'Вы в совершенстве владеете азами исторических наук. ' +
                            'Смело переходите на другие уровни сложности!',
                    },
                    // medium
                    {
                        bad: 'К сожалению, вы набрали низкое количество баллов. ' +
                            'Вам следует понизить уровень сложности.',
                        medium: 'Вы уже достаточно разбираетесь в истории. <br>' +
                            'Этот сайт поможет Вам закрыть дыры в материале!',
                        good: 'У вас уже есть опыт в изучении истории. ' +
                            'Этот сайт поможет вам продвинутся в этом направлении!',
                        awesome: 'Вы в совершенстве владеете азами исторических наук. ' +
                            'Смело повышайте уровень сложности!',
                    },
                    // hard
                    {
                        bad: 'К сожалению, вы набрали низкое количество баллов. ' +
                            'Вам следует понизить уровень сложности.',
                        medium: 'У вас уже есть опыт в изучении истории. ' +
                            'Этот сайт поможет вам продвинутся в этом направлении!',
                        good: 'Вы довольно долго изучаете историю. ' +
                            'Надеемся, этот сайт поможет в дальнейшем!',
                        awesome: 'Вы в совершенстве владеете историческими. ' +
                            'Смело переходите на очень сложный уровень!',
                    },
                    // extra hard
                    {
                        bad: 'Вы поставили невыполнимую планку для себя. ' +
                            'Вам следует понизить уровень сложности.',
                        medium: 'Вы очень долго изучаете историю. ' +
                            'Кажется, очень скоро вы станете экспертом!',
                        good: 'Судя по всему, вы невероятно увлечены историей. ' +
                            'Вам осталось совсем чуть-чуть, чтобы стать экспертом!',
                        awesome: 'Вы в совершенстве владеете историей! ' +
                            'Гордитесь, вы эксперт в данной области!',
                    }
                ];
            scoreElem.innerHTML = `${rightScore}/${allScore}.`;
            if (rightScore < 4) { // 1-3
                lvlElem.innerHTML = lvls[difficult].bad;
            } else if (rightScore < 7) { // 4-6
                lvlElem.innerHTML = lvls[difficult].medium;
            } else if (rightScore < 9) { // 7-8
                lvlElem.innerHTML = lvls[difficult].good;
            } else { // 9-10
                lvlElem.innerHTML = lvls[difficult].awesome;
            }
            _toggleDisplay(continueDisplay, endDisplay);
        }
        function makeAnswer() {
            try {
    ...
    [...document.querySelectorAll('.close-test-btn')].forEach(function(item) {
        item.addEventListener('click', test.close);
    })
